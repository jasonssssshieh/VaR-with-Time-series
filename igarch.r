...
cac <- array(as.numeric(unlist(c(data[2]))))
dow<- array(as.numeric(unlist(c(data[3]))))
nik<- array(as.numeric(unlist(c(data[4]))))
sp<-array(as.numeric(unlist(c(data[5]))))
...
data.d = cbind(cac,dow, nik, sp)
time = t(data[1])
#par(mfrow=c(2,2))   #一个图版显示2行，2列
for(i in 1 : 4){
  plot(data.d[,i], type = 'l', ylab = 'daily log return', xlab = 'date', main = colnames(data)[(i+1)])
}
...
adf.test(nik)
adf.test(sp)
#all of the data are stationary by ADF test
...
X = dow# you can change your data
n <- length(X)
##############################################################################################################################
##############################################################################################################################
...
plot(X, type = "l", xlim = xran, ylim = yran, xlab = "Time t", ylab = "",
     main = "Data, fitted AR-IGARCH process, VaR, VaR predictions and VaR CIs",
